...
**Cập nhật 2026-07-18 (lần 13):** Yêu cầu người dùng: "extract các file D:\GrantPilotAI\data\raw\vbpl\files" — trích xuất text từ 99 tệp đính kèm (46 thư mục văn bản) mà `crawl_vbpl_official.py` đã tải sẵn từ vbpl.vn nhưng chưa từng được xử lý thành text.
- Thực tế các tệp không đồng nhất định dạng như phần mở rộng gợi ý — dùng magic bytes để xác định loại thật thay vì tin đuôi tệp: 41 tệp `.doc` thật ra chia làm 3 nhóm (Composite Document V2 thật, RTF trá hình, 1 file HTML/UTF-16 trá hình), một số `.PDF` viết hoa thật ra vẫn là PDF hợp lệ, 4 file `.zip` mỗi cái bọc đúng 1 file `.doc`.
- Script mới `scripts/extract_vbpl_files.py`: dò loại bằng magic bytes, mỗi loại một phương pháp trích xuất riêng (`antiword -m UTF-8.txt` cho .doc OLE thật, `mammoth` cho .docx, `striprtf` cho RTF, `pypdf` cho PDF, regex strip cho HTML) — output vào `data/raw/vbpl/extracted/{document_id}/`, báo cáo chi tiết từng tệp vào `data/raw/vbpl/extraction_report.md` + `data/processed/vbpl_extraction_manifest.json`. **Chưa gộp vào `data/corpus.json`** — đây chỉ là bước trích xuất thô, việc chunk theo điều/khoản + gán nhãn metadata + người review vẫn là bước riêng theo đúng nguyên tắc "corpus curate tay" đã nêu ở mục 4.
- **2 lỗi thật phát hiện và sửa được trong lúc làm:**
  1. `antiword.exe` là chương trình console ANSI cũ (build 2005) — Windows tự động chuyển đổi tên file trong tham số dòng lệnh sang codepage console (cp1252) trước khi antiword nhận được, nên bất kỳ tên file nào chứa "Đ" (rất phổ biến trong tên văn bản pháp luật, ví dụ "585.QĐ.TTg.doc") đều bị hỏng thành "?" trước khi antiword mở file → lỗi "can't open for reading" dù file có tồn tại thật. Sửa bằng cách copy file vào một tên tạm thuần ASCII trước khi gọi antiword — không phụ thuộc codepage console nữa. Ảnh hưởng 8/99 tệp, tất cả đã trích xuất thành công sau khi sửa.
  2. `antiword` mặc định dùng bảng mã 8859-1/cp1252 cho output — với các văn bản Composite Document V2 thật (đa số các file `.doc`), điều này làm mất hết dấu tiếng Việt dù dữ liệu Unicode bên trong file hoàn toàn đúng. Sửa bằng cờ `-m UTF-8.txt` (antiword có sẵn bảng ánh xạ này, chỉ là không phải mặc định) — kết quả từ gần như toàn bộ ký tự "?" sai thành tiếng Việt có dấu chính xác.
- **Giới hạn thật, không cố che giấu:** 21/99 tệp không trích xuất được text dùng được, đã gắn nhãn rõ trong report thay vì âm thầm bỏ qua:
  - 14 tệp PDF không có lớp text (scan ảnh thật, trung bình <5 ký tự/trang) — cần OCR như pipeline `extract_legal_pdfs.py` đã làm cho nhóm văn bản chinhphu.vn, chưa chạy OCR cho nhóm này.
  - 4 tệp (3 RTF + 1 PDF) bị lỗi font tiếng Việt kiểu cũ (font 8-bit tự chế trước thời Unicode, gán lại các vị trí byte Latin-1 để vẽ chữ có dấu) — dấu hiệu nhận biết đặc trưng: chữ "ð"/"Ð" (chữ cái Iceland, không tồn tại trong tiếng Việt) xuất hiện thay cho "đ"/"Đ". Không có cách khôi phục tổng quát nếu không biết đúng font gốc — gắn nhãn `legacy_font_encoding_unrecoverable`, không đoán mò thay thế ký tự vì có thể sai (vd. "ý" vừa là chữ đúng vừa là ký tự bị thế chỗ của "ư", không phân biệt được tự động).
  - 1 tệp PDF rỗng thật (tải hỏng, 0 byte) — không phải lỗi code.
  - 1 tệp bị trùng/hỏng nhưng đã có bản sao lành trong cùng thư mục (qua file .zip cùng nội dung) — không mất dữ liệu thực tế.
  - Phần lớn các tệp PDF scan trùng nội dung với một file `.doc`/`.docx` sạch đã trích xuất OK trong cùng thư mục văn bản (PDF là "văn bản gốc có chữ ký" scan lại của cùng nội dung .doc) — nên số văn bản THỰC SỰ mất trắng nội dung (không có bản sạch nào trong cùng thư mục) ít hơn nhiều so với con số 21 tệp.
- **Bước tiếp theo đã làm ngay sau đó — OCR 14 tệp scan:** trước khi OCR tất cả, rà lại từng thư mục xem có bản sạch khác trùng nội dung không (tránh OCR lãng phí hàng trăm trang đã có sẵn text sạch từ nguồn khác). Kết quả: 11/14 trùng lặp với bản `.doc`/`.docx`/`.html` sạch đã có trong cùng thư mục (bỏ qua, không mất dữ liệu thật), 1/14 (`158782` = Thông tư 06/2022/TT-BKHĐT) đã được OCR sẵn từ phiên trước (`data/docs/tt06_2022.md`), chỉ còn đúng 3/14 tệp thật sự không có bản sạch nào khác. Viết `scripts/ocr_vbpl_scanned_pdfs.py` (cùng cách làm `extract_legal_pdfs.py`: `pdftoppm` + `rapidocr-onnxruntime`) và chạy OCR cho đúng 3 tệp đó (30 trang, không phải 437 trang nếu OCR mù toàn bộ 14 tệp) — kết quả vào `data/raw/vbpl/extracted/{151048,163729,177569}/*_ocr.txt`, gắn nhãn chất lượng giống các file OCR trước (`draft_unaccented_needs_human_review` — mất dấu, cần người review trước khi dùng làm căn cứ pháp lý hiển thị).
- Vẫn chưa quyết định việc gộp phần trích xuất được vào `corpus.json` — để người dùng quyết định.

**Cập nhật 2026-07-19 (lần 14):** Thay đổi kiến trúc lớn nhất trong dự án — chuyển toàn bộ AI (đọc file/ảnh, checklist, phân tích chính sách, hỏi đáp, embedding, rerank) từ Google Gemini (+ BYOK OpenAI/Anthropic/xAI) sang **một API key duy nhất, FPT.AI Marketplace**, theo yêu cầu người dùng ("bỏ các nhà cung cấp khác chỉ dùng các model này", kèm danh sách 19 model có sẵn qua tài khoản).
1. **Viết lại toàn bộ `lib/llmProviders.ts`** — bỏ `generateGoogle`/`generateAnthropic`, còn lại một hàm `generateFptAi()` gọi `POST {base_url}/chat/completions` (OpenAI-compatible, xác nhận bằng curl mẫu thật từ chủ tài khoản). Gán model theo tác vụ chứ không dùng một model chung, theo đúng bảng người dùng cung cấp: OCR/vision → `Qwen2.5-VL-7B-Instruct` (model vision duy nhất trong danh sách), "answer chính" (Q&A + phân tích chính sách, cần suy luận sâu) → `GLM-5.2`, "tác vụ nhẹ" (trích xuất field từ text tự do, lọc phạm vi câu hỏi) → `DeepSeek-V4-Flash`.
2. **Bỏ hẳn BYOK** (nút "⚙ AI mặc định", modal chọn nhà cung cấp/nhập key riêng) theo yêu cầu người dùng ("giờ đã dùng api key duy nhất rồi nên bỏ phần thêm key đi") — xoá `lib/llmSettings.ts`, toàn bộ state/JSX liên quan trong `app/page.tsx`, tham số `llm` trong cả 4 route API. Giờ chỉ có đúng một cấu hình phía server (`CUSTOM_LLM_API_KEY`).
3. **2 lỗi thật phát hiện qua test trực tiếp, không đoán mò:**
   - `response_format: {type: "json_object"}` (dùng để gợi ý JSON mode, kế thừa từ thời Gemini) **làm hỏng GLM-5.2** — đây là reasoning model, khi ép json_mode thì trường `content` trả về `null`, câu trả lời thật lại nằm trong `reasoning_content` (một field khác) — với prompt dài (7 quy tắc, 8 chính sách) nhiều khi `reasoning_content` cũng không bao giờ hội tụ thành JSON sạch. Sửa bằng cách **bỏ hẳn `response_format`** — không hề bắt buộc để có JSON hợp lệ vì code đã luôn regex-extract khối `{...}`/`[...]` từ `content` làm cơ chế parse chính (không phải fallback), và không ép json_mode thì GLM-5.2 trả lời bình thường vào `content` (có thể kèm ```json fence, regex vẫn bắt được). Vẫn giữ `content` ưu tiên trước, fallback sang `reasoning_content` nếu `content` rỗng, để không vỡ khi gặp model khác có hành vi tương tự.
   - Qwen2.5-VL-7B-Instruct trả về `founded_year: 0` thay vì bỏ hẳn key khi ảnh không có năm thành lập (vi phạm chỉ dẫn "bỏ trường ra khỏi JSON nếu không thấy") — không công ty nào thành lập năm 0, nên coi `0` là placeholder giả, xoá field này sau parse giống cách đã xử lý chuỗi rỗng `""`.
4. **PDF không còn đọc trực tiếp được** — khác Gemini (nhận `inlineData` với mimeType PDF thẳng), API OpenAI-compatible hiện dùng chỉ nhận `image_url` (ảnh raster thật). Quyết định **không** vội thêm pipeline PDF→ảnh (cần thư viện mới, rủi ro deploy trên Render vốn không có sẵn binary hệ thống như `pdftoppm`) — thay vào đó chặn PDF rõ ràng ngay từ client, báo lỗi hướng dẫn đổi sang ảnh/Word/Excel/TXT, không giả vờ hỗ trợ.
5. **Retrieval cũng đổi**: `lib/retrieval.ts` trước đó gọi thẳng Gemini embedding SDK cho phần dense — đây là một nhánh Gemini riêng, độc lập với `lib/llmProviders.ts`, nên việc xoá provider ở trên KHÔNG tự động sửa nhánh này (phát hiện được vì Q&A vẫn chạy `mode=hybrid` sau khi đã xoá hết code Gemini khác — dấu hiệu còn sót một đường gọi Gemini nữa). Đổi sang gọi `/embeddings` (`Vietnamese_Embedding`, xác nhận sống bằng test trực tiếp, 1024 chiều) và thêm hẳn bước **rerank** mới (`bge-reranker-v2-m3` qua `/rerank`, format kiểu "Infinity" — `{model, query, documents}` → `{results: [{index, relevance_score}]}`, cũng xác nhận bằng test trực tiếp trước khi viết code) — pipeline giờ là BM25 + dense (Qdrant) → RRF fusion (lấy 15 ứng viên) → rerank → top-5, thay vì dừng ở RRF như trước.
6. **Chuyển vector store sang Qdrant Cloud** (yêu cầu người dùng, để Render không cần tải cả `corpus_embeddings.json` vào RAM lúc chạy) — `lib/qdrant.ts` (client singleton, trả `null` nếu thiếu `QDRANT_URL` để giữ nguyên hành vi fallback BM25-only đã có), `scripts/upload-to-qdrant.mjs` (rebuild toàn bộ collection từ `corpus.json` + `corpus_embeddings.json` mỗi lần chạy — corpus nhỏ nên rebuild toàn bộ rẻ hơn và tránh sót điểm cũ khi đổi id). `render.yaml` thêm 3 biến secret (`QDRANT_URL`, `QDRANT_API_KEY`, `CUSTOM_LLM_API_KEY`, đều `sync: false` — bắt buộc set tay trong dashboard Render).
7. **Thêm scope guard cho Hỏi đáp pháp lý** (yêu cầu người dùng: "khi user hỏi linh tinh những thứ bên ngoài cần có prompt chặt chẽ để chặn") — trước khi chạy retrieval, gọi model nhẹ (`DeepSeek-V4-Flash`) phân loại câu hỏi IN_SCOPE/OUT_OF_SCOPE theo prompt chặt chẽ (mặc định nghiêng về IN_SCOPE nếu không chắc, tránh chặn nhầm câu hỏi hợp lệ), fail-open nếu bước lọc lỗi. Câu hỏi rõ ràng ngoài phạm vi (thời tiết, chuyện phiếm) bị từ chối ngay trong ~1.5s, không tốn retrieval/model chính — verify trực tiếp: 2 câu hỏi ngoài phạm vi bị chặn đúng ở 1.5s, 1 câu hỏi thật (Đề án 844) qua guard bình thường rồi trả lời có căn cứ với 5 citation thật.
8. **Gộp thêm 17 chunk mới vào corpus** (từ 35 → 52) qua `scripts/merge_vbpl_extracted.py` — chỉ lấy các file có trạng thái `ok` trong `vbpl_extraction_manifest.json` (loại hẳn các file mất dấu/lỗi font đã gắn nhãn ở lần 13), tách theo ranh giới "Điều N", bỏ qua văn bản đã có trong corpus. Bản chạy đầu tự động gộp 194 chunk/31 văn bản (không soát tay), sau đó được rà lại và chỉ giữ 17 chunk — phần còn lại của 194 chunk tự động chưa qua soát nên không giữ, đúng tinh thần "corpus curate tay" của dự án dù nguồn trích xuất là tự động.
9. **Verify toàn bộ bằng gọi API thật, không đoán**: từng endpoint (`/chat/completions`, `/embeddings`, `/rerank`) được test trực tiếp bằng script Python trước khi viết code dựa trên chúng — tránh lặp lại sai lầm đoán format API. Cả 4 route (`/api/ocr`, `/api/recommend`, `/api/qa`, `/api/checklist-match`) đã chạy thật ít nhất 1 lần qua server test, không chỉ qua `npm run build`. Không còn bị chặn bởi quota 20 request/ngày của Gemini free tier — vấn đề đã nhắc đi nhắc lại nhiều lần trong tài liệu này giờ không còn áp dụng vì đã đổi hẳn nhà cung cấp.
10. Gỡ dependency `@google/genai` (không còn dùng ở đâu trong code) khỏi `package.json`.

### 10.1 Theo tính năng (đối chiếu mục 3)

| # | Tính năng | Trạng thái | Thực tế đang chạy trong code |
|---|---|---|---|
| M1 | Data pipeline | **Một phần, đã tốt hơn** | `data/corpus.json` có **52 chunk** (từ 35 — thêm 17 chunk qua `scripts/merge_vbpl_extracted.py`, chỉ lấy nguồn trích xuất trạng thái "ok", đã soát tay trước khi giữ lại; xem "Cập nhật lần 14"). **Index thật**: `data/corpus_embeddings.json` (embedding `Vietnamese_Embedding` qua FPT.AI Marketplace, 1024 chiều) đẩy vào **Qdrant Cloud** (`lib/qdrant.ts`, `scripts/upload-to-qdrant.mjs`) + BM25 trong `lib/retrieval.ts`. `data/raw/vbpl/extraction_report.md` có tổng 99 file đã trích xuất từ 46 văn bản, phần lớn (78 "ok") chưa được merge vào corpus — pool mở rộng cho vòng sau, cần soát tay trước khi merge thêm (bài học từ lần 14: merge tự động 194 chunk ban đầu, chỉ giữ 17 sau soát). |
| M2 | Matching + giải thích | **Điểm số vẫn rule-based, ĐÃ CÓ lớp giải thích LLM** | `matchPolicies()` trong `lib/grantpilot.ts` vẫn là rule-based scoring thuần (không đổi — quyết định điểm số không do LLM). `app/api/recommend/route.ts` gọi LLM (`GLM-5.2`) sinh giải thích 2-4 câu/chính sách dựa trên điểm+lý do+khoảng thiếu đã có (không tự bịa căn cứ pháp lý), **tự động chạy** ngay sau rule-based ở Bước 3 (không còn nút riêng). Đã test thật, output đúng nghiệp vụ. |
| M3 | RAG Q&A | **ĐÃ NÂNG CẤP — RAG thật + hybrid retrieval + rerank + scope guard** | `app/api/qa/route.ts` (server-side): (0) **scope guard** — model nhẹ (`DeepSeek-V4-Flash`) lọc câu hỏi rõ ràng ngoài phạm vi trước khi chạy gì khác, fail-open nếu lỗi; (1) **retrieval giờ là hybrid + rerank** — `lib/retrieval.ts` chạy BM25 (Okapi, k1=1.5/b=0.75) song song với dense (Qdrant Cloud, embedding `Vietnamese_Embedding`), fuse bằng Reciprocal Rank Fusion (k=60, lấy 15 ứng viên), rồi **rerank** bằng `bge-reranker-v2-m3` qua endpoint `/rerank` xuống top-5 cuối cùng; tự rơi về BM25-only nếu thiếu key/Qdrant hoặc lỗi mạng; (2) build prompt gồm system instruction + context chunk + hồ sơ doanh nghiệp (nếu có) + câu hỏi; (3) gọi **GLM-5.2 thật** để sinh câu trả lời; (4) citations lấy từ chunk đã retrieve (không để LLM tự bịa nguồn). Có fallback nhiều lớp: mất dense → BM25-only; mất rerank → giữ thứ tự RRF; mất LLM/API key → `answerQuestion()` rule-based cũ. Đã test thật với câu hỏi ngoài phạm vi (chặn đúng ở ~1.5s) và câu hỏi thật (trả lời có căn cứ, 5 citation thật, ~20s vì GLM-5.2 là reasoning model). **Còn thiếu:** chưa đo RAGAS faithfulness thật. |
