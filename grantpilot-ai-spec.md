...
| LLM generate + OCR | API multimodal thương mại (1 chính + 1 dự phòng khác nhà cung cấp) | **Generate: ĐÃ CÓ, vượt yêu cầu ban đầu** — `lib/llmProviders.ts` hỗ trợ **4 nhà cung cấp** (Google Gemini, OpenAI, Anthropic Claude, xAI Grok), người dùng tự chọn + nhập key qua modal "Cài đặt AI" trên UI (lưu `localStorage`, không lưu server); mặc định dùng `GEMINI_API_KEY` phía server nếu người dùng không cấu hình. Có fallback rule-based khi mọi nhà cung cấp đều lỗi/không cấu hình. Đây chính là "1 chính + 1 dự phòng" mà spec yêu cầu, chỉ khác là dự phòng do **người dùng** chọn thay vì cấu hình cứng sẵn 2 key. **OCR: vẫn chưa có** — upload vẫn chỉ parse `.txt` thuần, chưa nối multimodal LLM. |
