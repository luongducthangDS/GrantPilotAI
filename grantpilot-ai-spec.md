...
| M5 | Grant assist (checklist + docx) | **Hoạt động thật** | Checklist tĩnh từ `policies.json` + API route `/api/grant-docx` dùng thư viện `docx` sinh file `.docx` thật — đã verify tải về và mở được. |
